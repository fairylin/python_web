...
def path_with_query(path, query):
    ...
    for k in query.keys():
        v = query[k]
        params_list.append('='.join([str(k), str(v)]))
...
def test_path_with_query():
    ...
    assert path_with_query(path, query) in expected, '请求的内容不再其中'
...
def test_header_from_dict():
    ...
    assert header_from_dict(headers) in headers_str, 'test_header_from_dict验证失败'
